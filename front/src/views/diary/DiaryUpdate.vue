<template>
  <div>
    ...
    <v-app-bar color="primary" dense dark>
      <v-app-bar-nav-icon @click="cancelForm"
        ><v-icon>mdi-chevron-left</v-icon></v-app-bar-nav-icon
      >
      <v-spacer></v-spacer>
      <v-toolbar-title style="font-family: 'Jua', sans-serif !important">{{
        createDate
      }}</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-btn
        color="primary"
        @click="checkForm"
        style="min-width: 48px; font-family: 'Jua', sans-serif !important"
        depressed
        class="pa-0"
        >수정</v-btn
      >
    ...
    <v-form ref="form" style="width: 300px" class="mx-auto">
      ...
      >
        <v-img
          :src="'http://k3a105.p.ssafy.io:8001/' + selectedImage"
          alt="selected_image"
          width="300"
          height="200"
          class="rounded-lg"
      ...
      <v-row
        v-else
        style="height: 200px; border: lightgray 2px solid; width: 300px"
        class="rounded-lg my-5 mx-auto"
        align="center"
        justify="center"
      >
        <v-icon size="60" color="lightgray" @click="showPhoto = true"
    ...
    <div v-if="showPhoto" style="position: absolute; left: 0; top: 0">
      <PhotoSelect
        id="picture_select"
        :images="images"
        @closePhoto="showPhoto = false"
        @selectPhoto="(val) => (selectedImage = val)"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getPostInfo() {
      http
        .get(`/api/post/${this.$route.params.id}`, this.config)
        .then((res) => {
          this.post = res.data;
          this.selectedImage = res.data.post_img;
          this.createDate =
            res.data.post_create.substring(0, 4) +
            "년 " +
            res.data.post_create.substring(5, 7) +
            "월 " +
            res.data.post_create.substring(8, 10) +
            "일";
          http
            .get(
              `/iot/day-imgs?choice_id=1002&day=${res.data.post_create.substring(
                0,
                10
              )}`,
              this.config
