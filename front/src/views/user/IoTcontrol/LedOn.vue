<template>
 <div class="nbn--font d-flex flex-column justify-space-between align-center" style="padding-top:20px">
    <v-img 
      src="@/assets/IoTcontrol/led_on.png"
      width="50%"
      v-show="!ledFlag"
    />
    ...
    <v-btn color="primary" outlined rounded block elevation="7" @click="onLED" :disabled="ledFlag">LED 켜기</v-btn>
    <v-col
      cols="12"
      sm="6"
      md="3"
  ...
  methods: {
    onLED() {
      ...
      if(this.user.choice_id == null){
        ...
        this.message = "LED를 켜고 있습니다."
        http
          .get("/iot/iot-actions?action=ledon&choice_id="+this.user.choice_id)
          .then(() => {
            this.ledFlag=false
            this.isDone = true
            this.message = "LED가 켜졌습니다."
            setTimeout(() => {
              this.isDone = false
            }, 5000);
          })
