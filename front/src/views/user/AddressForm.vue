<template>
  <v-container fluid class="fill-height">
    <v-row no-gutters class="align-self-start justify-center">
      <div class="text-h5 my-4 text-center">추가 정보</div>
    </v-row>
    <v-form ref="form">
      <v-row class="align-self-start px-8">
        <v-col cols="7">
          <v-text-field
            id="postcode"
            label="우편번호"
        ...
        <v-col cols="12">
          <v-btn @click="checkForm()" color="primary" outlined rounded block>
            회원가입
          </v-btn>
        </v-col>
        <div
...
export default {
  data() {
    return {
      ...
      addressRules: [(value) => !!value || "주소를 입력해주세요."],
      detailAddressRules: [(value) => !!value || "상세 주소를 입력해주세요."],
    };
  },
  computed: {
    ...mapState(["socialData"]),
  ...
  methods: {
    ...
    checkForm() {
      if (this.$refs.form.validate()) {
        var addr =
          this.addressData.address + " " + this.addressData.detailAddress;
